"""One-shot real R2021b probe using a structured JSON payload only."""
...
from src.coupling.stage4e_b1_v3_1_closeout.evidence import EventLog, ProcessEvidence, enumerate_matlab_processes, file_sha256, validate_event_log
...
def run_probe(*, project_root: str | Path = PROJECT_ROOT) -> dict[str, Any]:
    ...
    run = create_runtime_run(root, "stage4e_b1_probe_repair_v1")
    run_id = run.name
    token = f"r2021b_repair_{run_id}_{uuid.uuid4().hex}"
    logs, responses = run / "logs", run / "responses"
    payload_path = responses / "probe_payload.json"
    internal_log, console_log = logs / "matlab_internal.log", logs / "launcher_console.log"
    ...
    command = [str(MATLAB), "-wait", "-logfile", str(internal_log), "-batch", payload_expression(run_id=run_id, token=token, payload_path=payload_path)]
    result: dict[str, Any] = {
        "schema": "stage4e-b1-probe-repair-v1-result-1.0.0", "status": "environment_blocked",
        ...
        "return_code": None, "payload_validation": None, "owned_processes_started": [], "owned_processes_closed": [],
        "owned_residual_count": 0, "c_drive_project_artifacts": 0, "openfoam_started": 0,
    }
    event_log.append("preflight_completed", purpose="probe_repair", log_path=internal_log, payload={"preexisting_matlab_count": len(before_matlab), "command": command})
    evidence.start()
    process = None
    try:
        with console_log.open("w", encoding="utf-8") as console:
            ...
            row = evidence.register_pid(process.pid, purpose="matlab_probe_repair_launcher", log_path=internal_log)
            result["owned_processes_started"] = [row] if row else []
            event_log.append("probe_process_started", process=row, purpose="probe_repair", log_path=internal_log, payload={"command": command})
            result["return_code"] = process.wait(timeout=300.0)
            event_log.append("probe_process_exited", process=row, purpose="probe_repair", log_path=internal_log, exit_code=result["return_code"], payload={"payload_exists": payload_path.is_file()})
            if payload_path.is_file():
                payload = read_json_payload(payload_path)
                result["payload_validation"] = validate_payload(payload, runtime_root=run, return_code=result["return_code"], run_id=run_id, run_token=token, executable=MATLAB)
                if result["payload_validation"]["all_checks_passed"]:
                    result["status"] = "passed"
                else:
                    result["block_reason"] = "structured_probe_payload_checks_failed"
            else:
                result["block_reason"] = "structured_probe_payload_missing"
        ...
        result["owned_residual_count"] = sum(1 for row in evidence.snapshot_records() if row.get("pid") and _alive(int(row["pid"])))
    ...
    validation = result.get("payload_validation") or {}
    result["application_service_startup"] = bool(validation.get("checks", {}).get("application_service_ok"))
