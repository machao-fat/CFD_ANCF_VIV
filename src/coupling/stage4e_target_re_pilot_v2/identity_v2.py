"""Frozen parent identity and path-independent JSON helpers for B2-A-v2."""
...
FLOW_PATH = PROJECT / "results" / "08_stage4e_physical_baseline_v3_2_2" / "route_G_flow_profile_candidate.json"
...
def finite(value: Any) -> Any:
    ...
    if isinstance(value, float) and not math.isfinite(value):
        raise ValueError("NaN/Inf is not allowed in v2 evidence")
    return value
...
def load_formal_flow_profile(path: Path = FLOW_PATH) -> dict[str, Any]:
    flow = read_json(path)
    if flow.get("case_id") != EXPECTED_CASE_ID or flow.get("selected_candidate") != EXPECTED_CANDIDATE:
        raise ValueError("parent flow profile is not the frozen nine-slice candidate")
    if len(flow.get("slices", [])) != 9 or flow.get("flow_profile_sha256") != EXPECTED_FLOW_PROFILE_SHA256:
        raise ValueError("parent flow profile nine-slice identity/hash check failed")
...
def choose_representative_cases(flow: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    ordered = sorted(nonzero, key=lambda item: abs(float(item["signed_U_global_mps"])))
    selected = {"low": ordered[0], "middle": ordered[(len(ordered) - 1) // 2], "high": ordered[-1]}
    ...
    for label, item in selected.items():
        ...
        result[label] = {
            ...
            "pilot_U_mps": speed,
            "diameter_m": D,
            "kinematic_viscosity_m2ps": NU,
            "Re": speed * D / NU,
            "pilot_direction_policy": "positive_equivalent_magnitude; sign retained as provenance; reverse-flow boundary smoke is separate",
        }
    return result
