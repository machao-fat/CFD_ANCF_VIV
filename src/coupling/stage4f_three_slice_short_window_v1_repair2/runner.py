...
from ..stage4f_three_slice_preflight.campaign import ANCF_SOURCE, LowReStage4FRunner, _matlab_matrix, _matlab_quote
from .contract import D_M, END_TIME_S, PARENT_CASE_ROOT, Q_INF_PA, START_TIME_S, THRESHOLDS, TOTAL_SPAN_M, U_MPS, scaled_relative
from ..stage4f_c_applicationservice_repair_v2.probe import MATLAB_CORE, build_isolated_environment

REQUIRED_TIME_FIELDS = ("U", "p", "phi", "Uf", "meshPhi", "polyMesh/points", "uniform/time")
FLOAT = r"[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?"

# The OpenFOAM SIGFPE banner is informational.  Failure classification is
# intentionally split into explicit fatal/crash patterns and bounded numeric
# tokens so words such as ``information`` and ``infiniteLoop`` remain benign.
FOAM_FATAL_RE = re.compile(r"\bFOAM\s+FATAL(?:\s+IO)?\s+ERROR\b", re.IGNORECASE)
NEGATIVE_VOLUME_RE = re.compile(r"\bnegative\s+volume\b", re.IGNORECASE)
NONFINITE_TOKEN_RE = re.compile(
    r"(?<![A-Za-z0-9_])(?:nan|[+-]?inf(?:inity)?)(?![A-Za-z0-9_])", re.IGNORECASE,
)
FLOATING_POINT_CRASH_RE = re.compile(
    r"\bfloating\s+point\s+exception\b(?!\s+trapping\b)", re.IGNORECASE,
)
SIGFPE_CRASH_RE = re.compile(r"\b(?:received\s+signal|caught(?:\s+signal)?)\s+SIGFPE\b|\bSIGFPE\s*:\s*(?:abort|signal|crash)\b", re.IGNORECASE)
END_LINE_RE = re.compile(r"(?m)^\s*End\s*$")
...
class VariableStepRunner(LowReStage4FRunner):
    ...
    def _run(self, script: str, label: str) -> None:
        if not MATLAB_CORE.is_file():
            raise RuntimeError(f"MATLAB R2021b core is missing: {MATLAB_CORE}")
        log = self.work_dir / f"matlab_{self.index:03d}_{label}.log"
        self.index += 1
        self.logs.append(str(log))
        started_ns = time.time_ns()
        env = build_isolated_environment(self.work_dir / "matlab_environment")
        with log.open("w", encoding="utf-8") as stream:
            command = [str(MATLAB_CORE), "-batch", script]
            process = subprocess.Popen(command, cwd=str(self.work_dir), env=env, stdout=stream, stderr=subprocess.STDOUT)
            record = {"kind": "matlab", "label": label, "pid": process.pid, "started_ns": started_ns,
                "creation_time_utc": datetime.fromtimestamp(started_ns / 1.0e9, timezone.utc).isoformat().replace("+00:00", "Z"),
                "parent_pid": os.getpid(), "command_line": command,
                "cwd": str(self.work_dir), "log": str(log), "closed": False, "return_code": None}
            try:
                import psutil
                child = psutil.Process(process.pid)
                record.update({"creation_time": child.create_time(), "executable": child.exe(), "command_line_observed": child.cmdline(), "parent_pid_observed": child.ppid(), "environment_paths": {key: env.get(key) for key in ("TEMP", "TMP", "TMPDIR", "PREFDIR", "MATLAB_PREFDIR", "MATLAB_LOG_DIR")}})
            except Exception as exc:
                record["process_snapshot_error"] = type(exc).__name__
...
def _prepare_case_skeletons(root: Path, *, dt_s: float, start_time_s: float = START_TIME_S) -> dict[int, Path]:
    cases: dict[int, Path] = {}
    for sid in range(3):
...
def _log_audit(paths: Sequence[str], return_codes: Mapping[str, int | None] | Sequence[int | None] | None = None) -> dict[str, Any]:
    """Classify solver logs without treating the normal SIGFPE banner as fatal."""
    def code_for(index: int, path: Path) -> int | None:
        if return_codes is None:
            return None
        if isinstance(return_codes, Mapping):
            value = return_codes.get(str(path))
        else:
            value = return_codes[index] if index < len(return_codes) else None
    ...
    for index, item in enumerate(paths):
        ...
        reasons: list[str] = []
        fatal_matches = [match.group(0) for match in FOAM_FATAL_RE.finditer(text)]
        nonfinite_matches = [match.group(0) for match in NONFINITE_TOKEN_RE.finditer(text)]
        crash_matches = [match.group(0) for match in FLOATING_POINT_CRASH_RE.finditer(text)]
        sigfpe_matches = [match.group(0) for match in SIGFPE_CRASH_RE.finditer(text)]
        negative_volume_matches = [match.group(0) for match in NEGATIVE_VOLUME_RE.finditer(text)]
        if fatal_matches:
            reasons.append("foam_fatal")
        if nonfinite_matches:
            reasons.append("nonfinite_token")
        if crash_matches:
            reasons.append("floating_point_crash")
        if sigfpe_matches:
            reasons.append("floating_point_crash")
        if negative_volume_matches:
            reasons.append("negative_volume")
        return_code = code_for(index, path)
        if return_code is not None and return_code != 0:
            reasons.append("nonzero_return_code")
        has_end = bool(END_LINE_RE.search(text))
        row = {
            "path": str(path), "sha256": sha256_file(path), "has_End": has_end,
            "max_cfl": max(cfl) if cfl else None,
            "fatal_token": fatal_matches[0] if fatal_matches else None,
            "fatal_tokens": fatal_matches, "nonfinite_tokens": nonfinite_matches,
            "floating_point_crash_tokens": crash_matches,
            "sigfpe_crash_tokens": sigfpe_matches,
            "negative_volume_tokens": negative_volume_matches,
            "return_code": return_code, "failure_reasons": reasons,
        }
        row["passed"] = bool(has_end and row["max_cfl"] is not None
            and row["max_cfl"] < THRESHOLDS["cfl_strict_upper"] and not reasons)
        rows.append(row)
...
def run_segment(*, root: Path, branch: str, dt_s: float, first_step: int, step_count: int,
        ...
        process_registry: list[dict[str, Any]], start_time_s: float = START_TIME_S) -> dict[str, Any]:
    if root.exists():
        raise FileExistsError(root)
    root.mkdir(parents=True)
    manifest = _read_manifest()
    config = _runtime_config(manifest, dt_s)
    config = RuntimeConfig(schema_version=config.schema_version, case_id=config.case_id,
        dt_s=config.dt_s, timeout_s=config.timeout_s, start_time_s=start_time_s,
        coupling_iteration=config.coupling_iteration, coupling_scheme=config.coupling_scheme,
        slice_manifest_sha256=config.slice_manifest_sha256)
    source_payload = json.loads(source_checkpoint.read_text(encoding="utf-8"))
    cases = _prepare_case_skeletons(root, dt_s=dt_s, start_time_s=start_time_s)
    ...
    try:
        ...
        for spec in manifest.slices:
            ...
            process.preflight(format(start_time_s + dt_s, ".12g"))
        ...
        for step in range(first_step, first_step + step_count):
            current = start_time_s + (step - first_step) * dt_s
            target = current + dt_s
            ...
            return_codes = {str(path): process.return_code() for path, process in zip(new_logs, processes)}
            log_audit = _log_audit(new_logs, return_codes)
