...
void append_profile(std::int32_t global_step, std::uint32_t sequence,
                    const cfd_ancf::StepDiagnostics& diagnostics) {
  const char* path = std::getenv("CFD_ANCF_PROFILE_PATH");
...
int process_step(const std::vector<char>& payload, std::vector<char>& response,
  ...
  if (schema != SCHEMA || protocol != PROTOCOL || sequence == 0 || n <= 0 || elements < 2 || slices < 1 ||
  ...
  append_profile(global_step, sequence, diagnostics);
...
int main() {
  ...
  while (true) {
    ...
    const int result = process_step(payload, response, last_sequence + 1, expected_run, expected_case);
