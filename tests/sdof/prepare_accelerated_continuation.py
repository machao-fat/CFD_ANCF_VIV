...
def main() -> None:
    ...
    if args.target_case.exists() or args.target_result.exists():
        raise SystemExit("refusing to overwrite accelerated continuation target")
    ...
    payload = {
        "status": "accelerated_continuation_prepared_from_checkpoint",
        ...
        "forces_dat_preserved": True,
        "forces_function_object_console_log": False,
        "source_continuation_overwritten": False,
    }
    (args.target_result / "acceleration_preparation.json").write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
