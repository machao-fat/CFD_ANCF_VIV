...
const std::array<double, 9> kH = {1.0e-2, 3.0e-3, 1.0e-3, 3.0e-4, 1.0e-4,
...
double max_abs(const Matrix& value) {
  double result = 0.0;
  for (double item : value.data) result = std::max(result, std::abs(item));
  return result;
}
...
Model make_model() {
  ...
  model.boundary_contract_id = "tangent_fd_current_line_replacement_v1";
...
std::vector<double> make_direction(const Model& model, int mode) {
  ...
  for (std::size_t index = 0; index < direction.size(); ++index) {
    const double scale = index % 6 < 3 ? kLengthScale : kGradientScale;
    direction[index] /= normalization;
    if (scale == kLengthScale) direction[index] *= kLengthScale;
  }
  return direction;
}
...
DirectionRow directional_fd(const std::vector<double>& q, const Matrix& exact,
  ...
  const std::vector<double> fd_direction =
      [&]() {
        std::vector<double> result(force_plus.size(), 0.0);
        for (std::size_t index = 0; index < result.size(); ++index)
          result[index] = (force_plus[index] - force_minus[index]) / (2.0 * h);
        return result;
      }();
...
int main() {
  ...
  try {
    ...
    const Matrix exact_transpose = [&]() {
      Matrix result(exact.cols, exact.rows);
      for (std::size_t row = 0; row < exact.rows; ++row)
        for (std::size_t col = 0; col < exact.cols; ++col)
          result(col, row) = exact(row, col);
      return result;
    }();
    const Matrix symmetry_error = difference(exact, exact_transpose);
    const double exact_norm = frobenius(exact);
    const double symmetry_relative =
        frobenius(symmetry_error) / std::max(exact_norm, kScaleFloor);
    ...
    const bool full_region = best_index > 0 && best_index + 1 < full_rows.size();
    const bool full_pass = full_finite && full_region &&
                           best_it->relative_error <= kFullTolerance;
    ...
    for (std::size_t direction = 0; direction < directions.size(); ++direction) {
      ...
      const auto direction_best_it = std::min_element(
          begin, direction_rows.end(),
          [](const DirectionRow& left, const DirectionRow& right) {
            return left.relative_error < right.relative_error;
          });
      direction_min[direction] = direction_best_it->relative_error;
      direction_best[direction] = static_cast<std::size_t>(direction_best_it - begin);
      const bool this_finite = std::all_of(begin, direction_rows.end(), finite_direction);
      const bool this_region = direction_best[direction] > 0 &&
                               direction_best[direction] + 1 < kH.size();
      directional_finite = directional_finite && this_finite;
      directional_pass = directional_pass && this_finite && this_region &&
                         direction_min[direction] <= kDirectionalTolerance;
    }

    const bool pass = full_pass && directional_pass &&
                      symmetry_relative <= kSymmetryTolerance;
    std::cout << "{\"schema_version\":\"ANCF_TANGENT_FD_CURRENT_LINE_REPLACEMENT_V1\","
    ...
    std::cout << "],\"best_full_index\":" << best_index
              ...
              << ",\"full_finite\":" << (full_finite ? "true" : "false")
              << ",\"full_region\":" << (full_region ? "true" : "false")
              << ",\"directional_sweep\":[";
    ...
    std::cout << "],\"direction_min_relative_error\":[" << direction_min[0] << ","
              ...
              << "\"zero_damping\":true,\"external_load_in_api\":false,"
              << "\"production_source_modified\":false,\"finite_results\":"
              << ((full_finite && directional_finite && finite(symmetry_relative)) ? "true" : "false")
              << "}\n";
    return pass ? 0 : 1;
