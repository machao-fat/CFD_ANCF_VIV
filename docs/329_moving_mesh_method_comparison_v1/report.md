...
| 端木玉 RBFMotionSolver | - | - | not_tested | 当前仓库无 OpenFOAM 10 实现，不能冒充已测试 |
...
推荐将 `inverseDistance 1(cyl)` 作为下一阶段候选；保留 `quadratic inverseDistance 1(cyl)` 作为网格质量备选。RBF 需要单独移植 `RBFMotionSolver` 到 OpenFOAM 10 并重新做 smoke，当前不进入生产配置。
