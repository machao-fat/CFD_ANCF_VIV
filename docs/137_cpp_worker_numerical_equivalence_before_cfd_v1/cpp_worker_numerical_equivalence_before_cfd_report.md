...
Force semantics audit: MATLAB `state.last_slice_force_N` matches checkpoint `previous_slice_forces_N` exactly; the distinct `previous_applied_force_N` is not used for prediction. Prepared exporter (not executed): `tools/cpp_worker_numerical_equivalence_v1/export_step559_matlab_golden.m`. Running it requires a new explicit MATLAB authorization.
