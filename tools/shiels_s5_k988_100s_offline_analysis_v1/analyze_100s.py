...
def parse_events():
    ...
    # Preserve one row per physical time, with later parent/resume evidence winning
    by_time = {}
    for row in accepted:
...
def stable_zone(cycles):
    # A predeclared exploratory criterion: five cycles with <=5% CV in A and f,
    # and no monotonic trend larger than 5% of the five-cycle mean.
    for i in range(len(cycles) - 4):
        ...
        slope = abs(float(np.polyfit(np.arange(5), a, 1)[0])) * 4 / mean_a
        ...
        if cv_a <= 0.05 and cv_f <= 0.05 and slope <= 0.05 and cd_drift <= 0.05:
            return {"start_cycle_index": i, "end_cycle_index": i + 4,
                    "start_time_s": w[0]["t_start_s"], "end_time_s": w[-1]["t_end_s"],
                    "amplitude_cv": cv_a, "frequency_cv": cv_f, "amplitude_trend_fraction": slope,
