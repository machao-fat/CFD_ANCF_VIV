    ...
    for time_s in CONTRACT_VALUE["micro_smoke_evidence"]["mesh_snapshot_times_s"]:
        ...
        for sid, case in enumerate(cases):
            ...
            if time_s == 0.0:
                ...
                motion = rows[step - 1]["motion"][sid]
                displacement = [float(motion[f"u{axis}_m"]) for axis in "xyz"]
...
def main() -> int:
    ...
    for time_s in (0.05, 0.1):
        token = f"{time_s:g}"; us = [base.arr(case / token / "U", True) for case in cases]; ps = [base.arr(case / token / "p", False) for case in cases]
    ...
    max_co = max((float(record["courant_max"]) for item in quality.values() for record in item.get("time_records", [])), default=float("nan"))
