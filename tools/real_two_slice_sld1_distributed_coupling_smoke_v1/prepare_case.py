"""Prepare the isolated two-slice SLD1 OpenFOAM cases.

Only copied validation cases are written.  The supplied source case and all
...
def main() -> int:
    ...
    config = {
        "case_id": "real_two_slice_sld1_distributed_coupling_smoke_v1",
        "length_m": 10.0,
        "coupling": {
            "mode": "PiecewiseLinearDistributed",
            "placement": "explicit",
            "positions_m": [4.75, 5.25],
            "slice_ids": ["slice_0000", "slice_0001"],
            "slice_length_m": [1.0, 1.0],
            "unit_span_m": [1.0, 1.0],
            "active_start_m": 4.5,
            "active_end_m": 5.5,
    ...
    setup = {
        ...
        "generic_path": [
            ...
            "StructureCoordinator", "real preCICE", "OpenFOAM x2", "SLD1 persistent ANCF worker",
        ],
        "source_case": str(source),
        "generated_cases": [str(target) for target in targets],
        ...
        "copied_poly_mesh_sha256": {
            str(index): {
                rel: maybe_sha256(target / rel)
                for rel in KEY_FILES if rel.startswith("constant/polyMesh/")
            }
    ...
    print(json.dumps({
        "runtime": str(runtime), "manifest_sha256": manifest.manifest_sha256,
        "topology": topology, "cases": [str(target) for target in targets],
    }, ensure_ascii=False))
    return 0
