"""Validation-only Ns=2 real SLD1 structure participant.

This entrypoint deliberately composes the committed arbitrary-N manifest,
topology backend, generic gather/advance/scatter coordinator, and the current
persistent ANCF worker.  It is not production coupling code and contains no
case-specific numerical branch beyond the frozen smoke model contract.
"""
...
from coupling.cpp_worker_persistent_ipc_v1.kernel_protocol import (
    ...
    SPANWISE_ENDPOINT_POLICY_NEAREST_CONSTANT,
    SPANWISE_LOAD_MODE_PIECEWISE_LINEAR,
...
class PersistentKernelBackend:
    """One global ANCF state using the SLD1 worker request extension."""

    def __init__(self, manifest: SliceManifest, worker_path: str) -> None:
        if manifest.ns != 2 or manifest.reconstruction_mode != "PiecewiseLinearDistributed":
            raise RuntimeError("this bounded smoke requires Ns=2 PiecewiseLinearDistributed")
    ...
    def advance(self, request: WorkerRequest) -> Mapping[str, Any]:
        ...
        if not request.sld1 or request.reconstruction_mode != "PiecewiseLinearDistributed":
            raise RuntimeError("worker request is not SLD1 PiecewiseLinearDistributed")
        if len(request.spanwise_line_force_Npm) != 6:
            raise RuntimeError("SLD1 request does not contain two 3-component samples")
        q_before = self.q
        ...
        kernel_request = KernelStepRequest(
            ...
            dt_s=DT_S,
            request_id=920000 + step,
            transaction_id=1920000 + step,
            run_id="real-two-slice-sld1-smoke-v1",
        ...
        payload_sha = hashlib.sha256(kernel_request.payload()).hexdigest().upper()
        ...
        self.last_result = {
            ...
            "worker_request_payload_sha256": payload_sha,
    ...
    def close(self) -> dict[str, Any]:
        ...
        result.update({"return_code": self.process.returncode, "stderr": stderr,
...
def _force_sum(value: Any) -> tuple[float, float, float]:
    ...
    if not isinstance(rows, list) or len(rows) != VERTEX_COUNT:
        raise RuntimeError(f"preCICE force vertex count mismatch: {len(rows) if isinstance(rows, list) else 'non-list'} != {VERTEX_COUNT}")
    sums = [0.0, 0.0, 0.0]
    for row in rows:
...
def main() -> int:
    ...
    output: dict[str, Any] = {
        "task": "REAL_TWO_SLICE_SLD1_DISTRIBUTED_COUPLING_SMOKE_V1",
    ...
    try:
        ...
        if manifest.ns != 2 or manifest.reconstruction_mode != "PiecewiseLinearDistributed":
            raise RuntimeError("manifest must be exactly Ns=2 PiecewiseLinearDistributed")
        ...
        precice_backend = PreciceStructureFleetBackend(
            manifest, args.config, {item.slice_id: vertices for item in manifest.slices})
        ...
        for window in range(1, args.max_windows + 1):
            ...
            if write_checkpoint or read_checkpoint:
                raise RuntimeError("unexpected checkpoint action in bounded parallel-explicit SLD1 smoke")
            for item in manifest.slices:
                motion = coordinator.last_motion.get(item.slice_id, initial_motion[item.slice_id])
                precice_backend.write_motion(
                    item.slice_id, [[motion[0], motion[1]] for _ in range(VERTEX_COUNT)])
            precice_backend.advance(DT_S)
            # Deliberately read in reverse arrival/order order.  The generic
            # coordinator must still emit the final worker request in manifest order.
            ...
            for slice_id in read_order:
                ...
                raw_force = _force_sum(precice_backend.read_force(slice_id))
                ...
                samples[slice_id] = sample
                output["force_read_order"].append({"window": window, "slice_id": slice_id})
                coordinator.submit_force(sample)
            advance_result = coordinator.advance_if_complete()
            request = coordinator.last_request
            if request is None:
                raise RuntimeError("coordinator did not expose the SLD1 request")
        ...
        if output["accepted_windows"] < 3:
            raise RuntimeError("preCICE ended before frozen minimum of three accepted windows")
        output["status"] = "PASS"
    except Exception as exc:
        output["status"] = "FAIL"
