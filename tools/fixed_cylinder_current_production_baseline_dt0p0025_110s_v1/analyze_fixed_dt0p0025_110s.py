...
RUN_PARENT = ROOT / "runtime/fixed_cylinder_current_production_baseline_dt0p0025_60s_v1_run_004"
RUN_EXT = ROOT / "runtime/fixed_cylinder_current_production_baseline_dt0p0025_110s_v1_run_008"
RUN = RUN_EXT
CASE_PARENT = RUN_PARENT / "case"
CASE = RUN_EXT / "case"
OUT = ROOT / "results/fixed_cylinder_current_production_baseline_dt0p0025_110s_v1"
...
def parse_force_coeffs():
    paths = [
        CASE_PARENT / "postProcessing/cylinderForceCoeffs/0/forceCoeffs.dat",
        *sorted((CASE / "postProcessing/cylinderForceCoeffs").glob("*/forceCoeffs.dat")),
    ]
    rows = []
    for path in paths:
        if not path.exists():
    ...
    if not rows:
        raise RuntimeError("no force coefficient rows found in parent or continuation runtime")
    # Keep the continuation row at the shared 60 s boundary if times overlap.
...
def parse_forces():
    paths = [
        CASE_PARENT / "postProcessing/cylinderForces/0/forces.dat",
        *sorted((CASE / "postProcessing/cylinderForces").glob("*/forces.dat")),
    ]
    rows = {}
    for path in paths:
        if not path.exists():
...
def parse_quality():
    texts = []
    for path in [RUN_PARENT / "fluid.stdout", RUN_EXT / "fluid.stdout"]:
        if path.exists():
...
def parse_checkmesh():
    path = RUN_EXT / "checkMesh.stdout"
    if not path.exists():
        path = RUN_PARENT / "checkMesh.stdout"
...
def summarize_block(block):
    """Summarize a finite cycle block without labeling it stable."""
    if not block:
        return None
    mapping = {"St": "St", "mean_CD": "CD_mean", "CL_amplitude": "CL_amplitude_half_peak_to_peak", "CL_rms": "CL_rms"}
    out = {"cycle_count": len(block), "start_cycle": block[0]["cycle"], "end_cycle": block[-1]["cycle"],
           "start_time_s": block[0]["t_start_s"], "end_time_s": block[-1]["t_end_s"]}
    for name, field in mapping.items():
        vals = np.asarray([c[field] for c in block], float)
        mean = float(np.mean(vals)); std = float(np.std(vals, ddof=1)) if len(vals) > 1 else 0.0
        out[name] = {"mean": mean, "std": std, "CV": std / max(abs(mean), 1e-30), "values": vals.tolist()}
    out["trend_fraction"] = {}
    for name, field in [("A_over_D", "CL_amplitude_half_peak_to_peak"), ("St", "St"), ("mean_CD", "CD_mean")]:
        vals = np.asarray([c[field] for c in block], float)
        out["trend_fraction"][name] = float(abs(np.polyfit(np.arange(len(vals)), vals, 1)[0]) * max(len(vals)-1, 1) / max(abs(np.mean(vals)), 1e-30)) if len(vals) > 1 else 0.0
    return out
...
def plot_results(rows, cycles, stable, quality):
    ...
    for start, name, label in [(0.0, "lift_psd_full", "full 0--110 s"), (TRANSIENT_END, "lift_psd_stable", "t >= 20 s")]:
...
def list_time_dirs():
    by_time = {}
    # Prefer the continuation directory at the shared 60 s boundary, while
    # retaining parent fields for startup snapshots.
    for case in [CASE_PARENT, CASE]:
        for d in case.iterdir():
            if d.is_dir() and re.fullmatch(r"[-+]?\d+(?:\.\d+)?", d.name):
                by_time[round(float(d.name), 9)] = (float(d.name), d)
    return [by_time[k] for k in sorted(by_time)]
...
def render_snapshots():
    ...
    end = max(x[0] for x in dirs)
    targets = [max(0.0, 0.35 * end), 0.65 * end, 0.95 * end]; chosen = []
...
def build_report(metrics):
    ...
    terminal = metrics.get("terminal_block_statistics"); temporal = metrics.get("dt_sensitivity_comparison"); matched = metrics.get("matched_window_comparison")
    cls = "STABLE_PERIODIC_SHEDDING_WITH_CURRENT_CONFIGURATION_BIAS" if stable else "RESPONSE_NOT_YET_STATIONARY"
    stable_text = (f"Candidate stable zone: cycles {stable['start_cycle']}--{stable['end_cycle']} (t={stable['start_time_s']:.4g}--{stable['end_time_s']:.4g} s)." if stable else "Candidate stable zone: none under the predeclared five-cycle CV/trend criterion.")
    terminal_text = (f"The latest five-cycle block (cycles {terminal['start_cycle']}--{terminal['end_cycle']}, t={terminal['start_time_s']:.4g}--{terminal['end_time_s']:.4g} s) has means St={terminal['St']['mean']:.7g}, mean CD={terminal['mean_CD']['mean']:.7g}, CL amplitude={terminal['CL_amplitude']['mean']:.7g}; CVs are {terminal['St']['CV']:.3g}, {terminal['mean_CD']['CV']:.3g}, and {terminal['CL_amplitude']['CV']:.3g}. It coincides with the qualifying terminal stable zone when the predeclared gate is met." if terminal else "No finite terminal cycle block is available.")
    table = (f"| `St=fD/U` | {comp['St']['mean']:.8g} | 0.167 | {comp['St']['relative_error']:.6g} |\n| mean `CD` | {comp['mean_CD']['mean']:.8g} | 1.33 | {comp['mean_CD']['relative_error']:.6g} |\n| `CL` amplitude (half peak-to-peak) | {comp['CL_amplitude']['mean']:.8g} | 0.30 | {comp['CL_amplitude']['relative_error']:.6g} |" if comp else "| `St=fD/U` | n/a | 0.167 | n/a |\n| mean `CD` | n/a | 1.33 | n/a |\n| `CL` amplitude (half peak-to-peak) | n/a | 0.30 | n/a |")
    temporal_text = (f"For a matched terminal five-cycle window in each run, dt=0.0025 minus dt=0.005 gives ΔSt={matched['dt0p0025_minus_dt0p005']['St']:.7g} ({matched['relative_difference']['St']:.3%}), Δmean CD={matched['dt0p0025_minus_dt0p005']['mean_CD']:.7g} ({matched['relative_difference']['mean_CD']:.3%}), and ΔCL amplitude={matched['dt0p0025_minus_dt0p005']['CL_amplitude']:.7g} ({matched['relative_difference']['CL_amplitude']:.3%}). This is a finite-window temporal comparison, not a final convergence claim." if matched else (f"For descriptive temporal comparison, terminal finite-block mean minus the dt=0.005 s baseline stable-zone mean is ΔSt={temporal['terminal_minus_baseline']['St']:.7g}, Δmean CD={temporal['terminal_minus_baseline']['mean_CD']:.7g}, and ΔCL amplitude={temporal['terminal_minus_baseline']['CL_amplitude']:.7g}. Because this run has no qualifying stable zone, these are not final temporal-convergence errors." if temporal else "No dt=0.005 s baseline comparison was available."))
    return f"""# Fixed-cylinder current-production CFD baseline — dt=0.0025 s, 0--110 s

## Scope

The 0--60 s parent run and an independent 60--110 s continuation used the current 16244-cell production mesh. The sole numerical change relative to the dt=0.005 s baseline is `deltaT=0.0025 s`; no preCICE, Adapter, SDOF, ANCF, or FSI participant was loaded. Full fields were written every approximately 0.1 s and force coefficients every CFD step. The continuation starts from the parent run's accepted 60 s fields; the shared boundary sample is de-duplicated in the combined analysis.
...
- Binary identity: isolated OF10 owner-diagnostic prefix; `pimpleFoam` SHA-256 `{metrics['runtime_identity']['pimpleFoam_sha256']}`; Adapter/preCICE **not loaded**. Input SHA-256 manifest recorded separately.
- Historical `full30b` remains distinct (3268 cells, `icoFoam`, different settings); it is not substituted for this current-production baseline.

## Cycle statistics

Statistics discard startup `t<20 s`; complete cycles are detected from positive-going `C_L` zero crossings over the combined 0--110 s force history. {stable_text}
...
No literature-level conclusion is claimed unless a qualifying stable zone exists. The dt=0.005 s baseline had a stable fixed-cylinder lift amplitude above the Shiels fixed-cylinder reference, so this dt=0.0025 s continuation is intended to quantify temporal sensitivity rather than to explain the free-FSI lift deficit by assumption.

## Final conclusion

`FIXED_CYLINDER_CURRENT_PRODUCTION_BASELINE_DT0P0025_110S = {cls}`

The combined 0--110 s history is sufficient for a bounded temporal comparison. Any candidate stable zone is reported only when the predeclared five-cycle CV/trend gate is met; otherwise the terminal finite block is explicitly not called a stable zone. The fixed-cylinder lift amplitude is above, rather than below, the Shiels reference, so it does not support explaining the free-FSI lift deficit by a fixed-cylinder lift bias alone. Next priority: grid sensitivity after review of the matched temporal comparison. No further case is started automatically.
"""
...
def main():
    ...
    terminal_block = [c for c in cycles if c["t_start_s"] >= TRANSIENT_END][-5:]
    terminal_stats = summarize_block(terminal_block) if len(terminal_block) >= 5 else None
    cl_values = np.asarray([r["CL"] for r in DATA], float)
    _, _, psd_full_stats = psd(cl_values, 0.0)
    _, _, psd_post_transient_stats = psd(cl_values, TRANSIENT_END)
    baseline_path = ROOT / "results/fixed_cylinder_current_production_baseline_100s_v1/fixed_cylinder_current_production_baseline_100s_metrics.json"
    baseline = json.loads(baseline_path.read_text(encoding="utf-8")) if baseline_path.exists() else None
    dt_compare = None
    if baseline and terminal_stats and baseline.get("literature_comparison"):
        dt_compare = {"baseline_run_id": baseline.get("run_id"), "baseline_stable_zone": baseline.get("candidate_stable_zone"), "terminal_minus_baseline": {k: terminal_stats[k]["mean"] - baseline["literature_comparison"][k]["mean"] for k in ["St", "mean_CD", "CL_amplitude"]}}
    matched_compare = None
    baseline_cycles_path = ROOT / "results/fixed_cylinder_current_production_baseline_100s_v1/cycle_statistics.csv"
    if terminal_stats and baseline_cycles_path.exists():
        with baseline_cycles_path.open(encoding="utf-8", newline="") as fh:
            bcycles = [{k: (int(v) if k == "cycle" else float(v)) for k, v in row.items()} for row in csv.DictReader(fh)]
        bblock = [c for c in bcycles if c["t_start_s"] >= TRANSIENT_END and c["t_end_s"] <= 60.0][-len(terminal_block):]
        if len(bblock) == len(terminal_block):
            bs = summarize_block(bblock)
            matched_compare = {"baseline_run_id": baseline.get("run_id") if baseline else None, "window_s": [20.0, 60.0], "baseline_block": bs, "dt0p0025_block": terminal_stats,
                               "dt0p0025_minus_dt0p005": {k: terminal_stats[k]["mean"] - bs[k]["mean"] for k in ["St", "mean_CD", "CL_amplitude"]},
                               "relative_difference": {k: (terminal_stats[k]["mean"] - bs[k]["mean"]) / max(abs(bs[k]["mean"]), 1e-30) for k in ["St", "mean_CD", "CL_amplitude"]}}
    metrics = {
        "run_id": "FIXED_CYLINDER_CURRENT_PRODUCTION_BASELINE_DT0P0025_110S_V1_RUN_008",
        "parent_run_id": "FIXED_CYLINDER_CURRENT_PRODUCTION_BASELINE_DT0P0025_60S_V1_RUN_004",
        "continuation_interval_s": [60.0, 110.0],
        ...
        "literature_comparison": comp, "stable_statistics": stable_stats, "terminal_block_statistics": terminal_stats, "dt_sensitivity_comparison": dt_compare, "matched_window_comparison": matched_compare, "psd": {"full": psd_full_stats, "post_transient": psd_post_transient_stats}, "shiels_reference": SHIELS, "wake_pattern_visualization": wake,
        ...
        "runtime_identity": {
            ...
            "parent_fluid_return": (RUN_PARENT / "returns.txt").read_text(encoding="utf-8", errors="replace").strip() if (RUN_PARENT / "returns.txt").exists() else None,
        ...
        "free_fsi_bias_support": "NOT_ASSESSED_UNTIL_DT0P0025_FIXED_CYLINDER_METRICS_ARE_AVAILABLE",
    ...
    if comp:
        ...
        metrics["next_priority"] = "grid_sensitivity"
    fields = ["time_s", "Cm", "CD", "CL", "Fx_N", "Fy_N", "pressure_Fx_N", "pressure_Fy_N", "viscous_Fx_N", "viscous_Fy_N"]
    write_csv(OUT / "force_history.csv", DATA, fields); write_csv(OUT / "cycle_statistics.csv", cycles, list(cycles[0]) if cycles else ["cycle"]); dump_json(OUT / "fixed_cylinder_current_production_baseline_dt0p0025_110s_metrics.json", metrics); plot_results(DATA, cycles, stable, quality)
    (OUT / "FIXED_CYLINDER_CURRENT_PRODUCTION_BASELINE_DT0P0025_110S_V1_REPORT.md").write_text(build_report(metrics), encoding="utf-8")
