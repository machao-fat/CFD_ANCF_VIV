"""Prepare and launch the single authorized long bounded Shiels development run.

The run continues only the accepted 5.155 s one-period state with the same
...
RUNTIME = ROOT / "runtime" / "shiels_s5_k988_single_slice_free_fsi_long_development_v1_run_001"
RESULTS = ROOT / "results" / "shiels_s5_k988_single_slice_free_fsi_long_development_v1_run_001"
PARENT_RUNTIME = ROOT / "runtime" / "shiels_s5_k988_single_slice_free_fsi_one_period_extension_v1_run_002"
PARENT_CASE = PARENT_RUNTIME / "precice_displacementLaplacian"
PARENT_RESULT = ROOT / "results" / "shiels_s5_k988_single_slice_free_fsi_one_period_extension_v1_run_002" / "first_trial_result.json"
PYDEPS = ROOT / "runtime" / "284_precice_single_slice_smoke_real_v1" / "python_deps"
PARTICIPANT = ROOT / "tools" / "shiels_s5_k988_single_slice_free_fsi_long_development_v1" / "sdof_precice_continuation_participant.py"
...
def source_hashes() -> dict[str, str]:
    ...
    return {name: sha256(PARENT_CASE / "5.155" / name) for name in names}
...
def require_parent() -> dict[str, Any]:
    required = (
        "5.155/U", "5.155/p", "5.155/phi", "5.155/Uf", "5.155/meshPhi", "5.155/pointDisplacement",
        "5.155/cellDisplacement", "5.155/polyMesh/points", "5.155/uniform/time", "constant/polyMesh/points",
    ...
    if missing:
        raise RuntimeError(f"accepted 5.155 CFD continuation state is incomplete: {missing}")
    if (PARENT_RUNTIME / "returns.txt").read_text(encoding="utf-8").strip() != "structure=0 fluid=0":
        raise RuntimeError("parent run did not have two successful process returns")
    parent_result = json.loads(PARENT_RESULT.read_text(encoding="utf-8"))
    if parent_result.get("status") != "PASS" or not all(parent_result.get("hard_gates", {}).values()):
        raise RuntimeError("parent one-period extension qualification is not PASS")
    ...
    if len(commits) != 1000:
        raise RuntimeError("parent does not contain exactly 1000 accepted commits")
    final = commits[-1]
    if abs(float(final["physical_time_s"]) - START) > 1.0e-12:
        raise RuntimeError("parent final commit is not at the authorized 5.155 s boundary")
    state = final["accepted_state"]
    if int(state["step"]) != 1010 or abs(float(state["local_time_s"]) - 5.05) > 1.0e-12:
        raise RuntimeError("parent SDOF state has an unexpected 5.155 s step/time identity")
    force_file = PARENT_CASE / "postProcessing" / "cylinderForces" / "0.155" / "forces.dat"
...
def prepare() -> None:
    ...
    contract = {
        "schema_version": "shiels-s5-k988-single-slice-free-fsi-long-development-v1",
        "execution_policy": "one controlled run only to 100.155 s; automatic retry/extension forbidden",
        "physical_contract": {
        ...
        "restart": {
            "parent_runtime": str(PARENT_RUNTIME), "parent_result_sha256": parent["parent_result_sha256"],
    ...
    write_json(RUNTIME / "manifest.json", {
        ...
        "parent_5p155_hashes": source_hashes(), "restart_force": initial_force, "case": str(case),
...
def preflight() -> None:
    ...
    parent = require_parent()
    case = RUNTIME / "precice_displacementLaplacian"
    copied_hashes = {name: sha256(case / "5.155" / name) for name in source_hashes()}
    ...
    outcome = {"continuation_wrapper_unit": unit, "unit_stdout": done.stdout, "unit_stderr": done.stderr, "abi": abi,
               "parent_restart": parent, "copied_5p155_field_identity": copy_identity,
               "time_layer": {"initial_data": "write parent final CFD interface y(t0=5.155) before initialize", "trial": "write candidate y(t_n), advance, read Force(t_n), correct", "rollback": "restore the same long-run window SDOF state/previous accepted force only"}}
...
def audit() -> dict[str, Any]:
    ...
    result = {"schema_version": "shiels-s5-k988-single-slice-free-fsi-one-period-extension-v1-result", "status": "PASS" if all(hard.values()) else "FAIL_CLOSED",
