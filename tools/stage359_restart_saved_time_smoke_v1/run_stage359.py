"""Run one fresh Smoke from the actual saved field time, then stop."""
...
impl.SOURCE_RUNTIME = ROOT / "runtime/stage358_restart_saved_time_alignment_v1_fresh"
...
impl.SMOKE_RUNTIME = ROOT / "runtime/stage359_restart_saved_time_smoke_v1"
impl.SMOKE_RESULTS = ROOT / "results/359_restart_saved_time_smoke_v1"
impl.CONT_RUNTIME = ROOT / "runtime/stage359_continuation_forbidden"
impl.CONT_RESULTS = ROOT / "results/359_continuation_forbidden"
...
impl.SMOKE_RUN_ID = "run359_restart_saved_time_smoke_v1"
impl.SMOKE_CASE_ID = "case359_restart_saved_time_smoke_v1"
impl.CONT_RUN_ID = "run359_continuation_forbidden"
impl.CONT_CASE_ID = "case359_continuation_forbidden"


def verify_saved_time_source():
    for path in (impl.SOURCE_STATE, impl.BOOTSTRAP_STATE, impl.FIXTURE, impl.WORKER, impl.PARTICIPANT, impl.QUALITY):
        if not path.is_file():
    ...
    if bootstrap.source_global_step != 16000 or abs(bootstrap.state_time_s - 79.995) > 1e-12:
        raise RuntimeError("Stage350 state is not the expected 79.995 s bootstrap")
    for index in range(3):
        restart = impl.SOURCE_RUNTIME / f"slice_{index:04d}" / "79.995"
        if not all((restart / name).is_file() for name in ("U", "p", "pointDisplacement", "cellDisplacement", "phi", "meshPhi")):
            raise RuntimeError(f"missing saved-time field in {restart}")
    return {
