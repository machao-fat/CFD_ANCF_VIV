"""Prepare, run once, and audit one bounded response-period Shiels extension.

The test continues only the accepted 0.155 s first-trial state with the same
...
RUNTIME = ROOT / "runtime" / "shiels_s5_k988_single_slice_free_fsi_one_period_extension_v1_run_002"
RESULTS = ROOT / "results" / "shiels_s5_k988_single_slice_free_fsi_one_period_extension_v1_run_002"
PARENT_RUNTIME = ROOT / "runtime" / "shiels_s5_k988_single_slice_free_fsi_first_trial_v1_run_002"
PARENT_CASE = PARENT_RUNTIME / "precice_displacementLaplacian"
PARENT_RESULT = ROOT / "results" / "shiels_s5_k988_single_slice_free_fsi_first_trial_v1_run_002" / "first_trial_result.json"
PYDEPS = ROOT / "runtime" / "284_precice_single_slice_smoke_real_v1" / "python_deps"
PARTICIPANT = ROOT / "tools" / "shiels_s5_k988_single_slice_free_fsi_one_period_extension_v1" / "sdof_precice_continuation_participant.py"
...
def source_hashes() -> dict[str, str]:
    ...
    return {name: sha256(PARENT_CASE / "0.155" / name) for name in names}
...
def require_parent() -> dict[str, Any]:
    required = (
        "0.155/U", "0.155/p", "0.155/phi", "0.155/Uf", "0.155/meshPhi", "0.155/pointDisplacement",
        "0.155/cellDisplacement", "0.155/polyMesh/points", "0.155/uniform/time", "constant/polyMesh/points",
    ...
    if missing:
        raise RuntimeError(f"accepted 0.155 CFD continuation state is incomplete: {missing}")
    ...
    if parent_result.get("status") != "PASS" or not all(parent_result.get("hard_gates", {}).values()):
        raise RuntimeError("parent first-trial qualification is not PASS")
    ...
    if len(commits) != 10:
        raise RuntimeError("parent does not contain exactly ten accepted commits")
    final = commits[-1]
    if abs(float(final["physical_time_s"]) - START) > 1.0e-12:
        raise RuntimeError("parent final commit is not at the authorized 0.155 s boundary")
    state = final["accepted_state"]
    if int(state["step"]) != 10 or abs(float(state["local_time_s"]) - 0.05) > 1.0e-12:
        raise RuntimeError("parent SDOF state has an unexpected step/time identity")
    force_file = PARENT_CASE / "postProcessing" / "cylinderForces" / "0.105" / "forces.dat"
...
def prepare() -> None:
    ...
    contract = {
        "schema_version": "shiels-s5-k988-single-slice-free-fsi-one-period-extension-v1",
    ...
    write_json(RUNTIME / "manifest.json", {
        ...
        "parent_0p155_hashes": source_hashes(), "restart_force": initial_force, "case": str(case),
...
def preflight() -> None:
    ...
    parent = require_parent()
    case = RUNTIME / "precice_displacementLaplacian"
    copied_hashes = {name: sha256(case / "0.155" / name) for name in source_hashes()}
    ...
    outcome = {"continuation_wrapper_unit": unit, "unit_stdout": done.stdout, "unit_stderr": done.stderr, "abi": abi,
               "parent_restart": parent, "copied_0p155_field_identity": copy_identity,
               "time_layer": {"initial_data": "write parent final CFD interface y(t0=0.155) before initialize", "trial": "write candidate y(t_n), advance, read Force(t_n), correct", "rollback": "restore the same extension-window SDOF state/previous accepted force only"}}
