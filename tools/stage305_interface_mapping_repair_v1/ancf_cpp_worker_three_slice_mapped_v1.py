...
def main() -> int:
    ...
    parser.add_argument("--allow-qualification-window", action="store_true",
                        help="allow an explicitly qualified dt/window; default remains the 0.005 s smoke contract")
    args = parser.parse_args()
    if args.source_step < 0 or args.source_time < 0 or args.steps <= 0 or args.dt <= 0:
        raise SystemExit("continuation requires non-negative source identity, positive steps, and dt > 0")
    if not args.allow_qualification_window and args.dt != 0.005:
        raise SystemExit("default smoke contract requires dt=0.005; pass --allow-qualification-window for an authorized qualification")
