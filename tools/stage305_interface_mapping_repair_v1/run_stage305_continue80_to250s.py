"""Stage 305 authorized continuation: source 80 s -> target 250 s."""
...
RUNTIME = ROOT / "runtime/stage305_interface_mapping_repair_v1_continue80_to250s"
RESULTS = ROOT / "results/305_interface_mapping_repair_v1"
...
INITIAL_STATE = ROOT / "runtime/stage304_interface_mapping_repair_v1_fresh_zero_to80s/logs/structure_participant.json"
...
LOGS = RUNTIME / "logs"
RUN_ID = "s305_continue80_to250s_mapping_diag_v1"
CASE_ID = "c305_continue80_to250s_mapping_diag_v1"
DT = 0.005
SOURCE_STEP = 16000
SOURCE_TIME = 80.0
STEPS = 34000
...
def prepare() -> list[Path]:
    ...
    for index in range(3):
        ...
        for name in ("0", "80", "constant", "system"):
...
def main() -> int:
    ...
    checks = {
        "source_step_16000_time_80": structure.get("source_global_step") == SOURCE_STEP and structure.get("source_time_s") == SOURCE_TIME,
        "finalized": structure.get("finalized") is True,
        "committed_50000": structure.get("target_global_step") == SOURCE_STEP + STEPS and structure.get("local_committed_steps") == STEPS,
        "slice_counts_34000": all(structure.get("slice_counts", {}).get(f"slice_{index:04d}") == STEPS for index in range(3)),
        "mapping_diagnostics_34000": len(diagnostics) == STEPS,
    ...
    gate = {
        "gate_id": "STAGE4F_D_INTERFACE_MAPPING_REPAIR_V1_CONTINUE80_TO250S_GATE",
        "status": "pass" if run.returncode == 0 and all(checks.values()) else "do_not_pass",
        "stage_id": "stage4f_d_interface_mapping_repair_v1_continue80_to250s",
        "run_id": RUN_ID,
        "case_id": CASE_ID,
        "scope_contract": {"source_step": SOURCE_STEP, "source_time_s": SOURCE_TIME, "target_step": SOURCE_STEP + STEPS, "target_time_s": SOURCE_TIME + STEPS * DT, "dt_s": DT, "slice_count": 3, "storage": "rolling fields + scalar convergence and mapping diagnostics"},
        "checks": checks,
        ...
        "protected": {"stage302_runtime_modified": False, "stage303_runtime_modified": False, "historical_evidence_modified": False, "ancf_eb_core_modified": False, "physical_parameters_modified": False, "global_dt_modified": False, "numerical_thresholds_modified": False, "formal_protocol_modified": False},
        "qualification": "continuation from verified 80 s state to 250 s with corrected interface mapping and scalar convergence observability",
    ...
    (RESULTS / "stage4f_d_interface_mapping_repair_v1_continue80_to250s_gate.json").write_text(json.dumps(gate, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
