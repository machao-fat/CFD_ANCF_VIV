...
%EXPORT_STEP560_MATLAB_INTERMEDIATE_TRACE Stage185 bounded MATLAB export.
% This script is the only MATLAB entry point for Stage185. It searches the
...
if nargin ~= 1 || ~(ischar(output_json) || isstring(output_json))
    error('stage185:Arguments', 'output_json is required');
end
...
if numel(matches) ~= 1
    error('stage185:SourceAmbiguous', 'expected exactly one matching source, found %d', numel(matches));
end
...
if source_step ~= 559 || abs(source_time - 2.2075) > 1e-12 || source_tick ~= 2207500000
    error('stage185:SourceIdentity', 'source identity mismatch');
end
if ~isfield(state, 'last_slice_force_N')
    error('stage185:ForceMissing', 'source slice force is missing');
end
slice_force = state.last_slice_force_N;
if size(slice_force, 1) ~= numel(model.coupling.s_ref_m) || size(slice_force, 2) ~= 3
    error('stage185:ForceSchema', 'slice force dimensions mismatch');
end
...
if double(state_after.step) ~= 560 || abs(target_time-2.20875) > 1e-12 || target_tick ~= 2208750000
    error('stage185:TargetIdentity', 'target identity mismatch');
end

trace = struct();
trace.schema_version = 1;
trace.trace_layout = 'float64 JSON arrays; vectors column-major; element ascending; Gauss ascending';
trace.stage_id = 'stage4f_d_cpp_worker_matlab_intermediate_trace_repair_v1';
trace.run_id = 'cpp_worker_matlab_intermediate_trace_001';
trace.case_id = 'cpp_worker_matlab_intermediate_trace_case_001';
...
trace.global_dt = dt;
trace.request_id = 1850005601;
trace.transaction_id = 1850005601;
trace.q_source = q_n.';
...
if fid < 0, error('stage185:Output', 'cannot open output'); end
cleanup = onCleanup(@() fclose(fid));
fwrite(fid, [encoded newline], 'char');
clear cleanup;
if ~movefile(tmp, output_json, 'f')
    error('stage185:OutputRename', 'atomic output rename failed');
end
...
if fid < 0, error('stage185:Manifest', 'cannot open manifest'); end
...
for ie=1:ne
    ...
    for k=1:numel(xi)
        ...
        if a2<1e-24,error('stage185:Degenerate','degenerate ANCF tangent');end
...
fid=fopen(path,'r','n'); if fid<0,error('stage185:Read','cannot read output');end
