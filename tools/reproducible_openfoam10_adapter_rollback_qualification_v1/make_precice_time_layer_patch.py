"""Generate the OF10 adapter candidate with explicit preCICE time-layer reads.

The patch is deliberately based on baseline + diagnostics, never on rejected
patch 0003.  Initial data uses the window-start sample; a retry after
``advance()`` reads the end-of-window sample for the next tentative solve.
"""
...
def main() -> int:
    if len(sys.argv) != 5:
        raise SystemExit("usage: make_precice_time_layer_patch.py <source> <work> <patch-0002> <output>")
    ...
    if work.exists() or diagnostic.exists() or output.exists():
        raise RuntimeError("refusing to overwrite time-layer work or patch")
    ...
    text = replace_once(
        ...
        "    // Once a trial has created it, normal API access reconstructs its\n"
