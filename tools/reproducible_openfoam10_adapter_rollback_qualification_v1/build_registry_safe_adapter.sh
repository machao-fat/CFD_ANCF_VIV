#!/usr/bin/env bash
# Build a new OF10 registry-safe candidate; never applies rejected patch 0003.
set -eo pipefail
...
safe_patch="$script_dir/patches/0004-registry-safe-rollback-and-motion-timing.patch"
[[ -f "$archive" && -f "$base_patch" && -f "$diag_patch" && -f "$safe_patch" && ! -e "$build_root" ]] || exit 2
mkdir -p "$build_root"
...
patch -d "$source_dir" -p1 < "$safe_patch"
