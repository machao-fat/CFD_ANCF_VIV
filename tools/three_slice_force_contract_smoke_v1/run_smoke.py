...
def launch(cases: list[Path]) -> int:
    ...
    script = ["set -uo pipefail", "source /opt/openfoam10/etc/bashrc", f"export PYTHONPATH='{root}/src:{wsl(PYDEPS)}'", f"python3 '{part}' --contract '{wsl(RUNTIME / CONTRACT.name)}' --state '{wsl(STATE)}' --worker '{wsl(WORKER)}' --runtime '{wsl(RUNTIME)}' --config {configs} --vertex-count 40 > '{wsl(logs / 'structure.stdout')}' 2> '{wsl(logs / 'structure.stderr')}' & spid=$!", *fluid, f"printf 'structure_pid=%s\\nfluid_0000_pid=%s\\nfluid_0001_pid=%s\\nfluid_0002_pid=%s\\n' \"$spid\" \"$p0\" \"$p1\" \"$p2\" > '{wsl(logs / 'pids.txt')}'", "wait \"$spid\"; sr=$?; if [ \"$sr\" -ne 0 ]; then kill \"$p0\" \"$p1\" \"$p2\" 2>/dev/null || true; fi", "wait \"$p0\"; r0=$?; wait \"$p1\"; r1=$?; wait \"$p2\"; r2=$?", f"printf 'structure_return=%s\\nfluid_0000_return=%s\\nfluid_0001_return=%s\\nfluid_0002_return=%s\\n' \"$sr\" \"$r0\" \"$r1\" \"$r2\" > '{wsl(logs / 'returns.txt')}'", "[ \"$sr\" -eq 0 ] && [ \"$r0\" -eq 0 ] && [ \"$r1\" -eq 0 ] && [ \"$r2\" -eq 0 ]"]
    completed = subprocess.run(["wsl.exe", "-d", "Ubuntu-22.04", "--", "bash", "-lc", "; ".join(script)], cwd=ROOT, text=True, capture_output=True, encoding="utf-8", errors="replace", timeout=900)
...
def audit(code: int) -> dict[str, object]:
    ...
    for sid in range(3):
        ...
        for line in (RUNTIME / "logs" / f"fluid_{sid:04d}.stdout").read_text(encoding="utf-8",errors="replace").splitlines(): parser.feed(line)
