...
def quality_mesh(case: Path, token: str, label: str, sid: int) -> dict[str, object]:
    ...
    return {"return_code": done.returncode, "min_cell_volume": one(r"Min volume\s*=\s*([-+0-9.eE]+)"),
            "max_non_orthogonality": one(r"Mesh non-orthogonality Max:\s*([-+0-9.eE]+)"),
            "max_skewness": one(r"Max skewness\s*=\s*([-+0-9.eE]+)"),
...
def collect(label: str, launch: dict[str, object]) -> dict[str, object]:
    ...
    return {"launch": launch, "gate": gate, "committed_windows": len(records), "final_openfoam_time_s": 0.1 + max((float(row["time_s"]) for row in records), default=0.0), "ledger": ledger, "mesh_quality": mesh, "maxima": maxima, "quality_v4": quality, "algorithmic_interface_work_J": work, "first_blocker": first_blocker}
...
def report(manifest: dict[str, object], explicit: dict[str, object], implicit: dict[str, object], comparison: dict[str, object], classification: str) -> None:
    def describe(name: str, item: dict[str, object]) -> str:
        return f"- {name}: committed `{item['committed_windows']}/10`; final OF time `{item['final_openfoam_time_s']:.12g}` s; gate `{item['gate'].get('FORMAL_IMPLICIT_ONE_WINDOW', 'NOT_EVALUABLE')}`; first blocker `{item['first_blocker']}`."
    lines = ["# SAME_BASELINE_EXPLICIT_IMPLICIT_0P05S_DIAGNOSTIC_V1_REPORT", "", "## Frozen baseline", "", f"- Git commit: `{manifest['git']['commit']}`; dirty at freeze: `{manifest['git']['dirty']}`.", f"- Adapter: `{manifest['adapter']['realpath']}`, SHA256 `{manifest['adapter']['sha256']}`; upstream `{manifest['adapter']['upstream_commit']}`; patches `{manifest['adapter']['patch_set']}`.", "- Both cases use `PRECURSOR_STATE_V1`, no-flow equilibrium, dt=0.005 s, OF time 0.100→0.150 s, Quality V4, and Generalized Force V2. The only intended variable is coupling scheme.", "", "## Run status", "", describe("parallel-explicit", explicit), describe("parallel-implicit", implicit), "", "## Comparison", "", f"- Common committed coupling times [s]: `{comparison['common_times_s']}`.", f"- Algorithmic interface work (not a physical-energy claim), explicit [J]: `{explicit['algorithmic_interface_work_J']}`; implicit [J]: `{implicit['algorithmic_interface_work_J']}`.", f"- Explicit maxima: `{explicit['maxima']}`.", f"- Implicit maxima: `{implicit['maxima']}`.", f"- Explicit Quality V4: `{explicit['quality_v4']}`.", f"- Implicit Quality V4: `{implicit['quality_v4']}`.", f"- Classification: `{classification}`.", "", "## Scope", "", "- This is a short numerical coupling comparison only. It makes no VIV, lock-in, long-time amplitude, or added-mass conclusion.", "- No run longer than the two authorized 0.050 s cases was launched."]
    put(DOC, "\n".join(lines) + "\n")
...
def main() -> int:
    ...
    result = {"baseline_manifest": manifest, "explicit": explicit, "implicit": implicit, "common_time_comparison": comparison, "classification": classification, "NEXT_SHORT_TIME_EXTENSION": "CONDITIONAL" if classification == "BOTH_SHORT_HORIZON_PASS" else "NOT_AUTHORIZED"}
    put(RESULTS / "paired_diagnostic.json", result); report(manifest, explicit, implicit, comparison, classification)
