...
def collect(label: str, launch: dict[str, object]) -> dict[str, object]:
    ...
    attempts_path = runtime / "implicit_iterations.jsonl"
    attempts = ([json.loads(line) for line in attempts_path.read_text(encoding="utf-8").splitlines()]
                if attempts_path.is_file() else [])
    metrics = [row["generalized_force_metric_v2"] for row in attempts if row.get("event") == "post_cpp_correction"]
    gf_v2 = {"status": "PASS" if metrics and all(metric.get("GENERALIZED_FORCE_MAPPING_V2") == "PASS" for metric in metrics) else "FAIL",
             "max_threshold_utilization": max((float(metric["max_threshold_utilization"]) for metric in metrics), default=None),
             "attempt_count": len(metrics)}
    committed_newton = [record for row in records for record in row.get("newton_evidence", [])]
    newton = {"status": "PASS" if len(committed_newton) == 2 * len(records) and all(record.get("newton_converged") for record in committed_newton) else "FAIL",
              "committed_record_count": len(committed_newton)}
    ...
    return {"launch": launch, "gate": gate, "structure_summary": summary,
            "committed_windows": len(records), "final_openfoam_time_s": 0.1 + max((float(row["time_s"]) for row in records), default=0.0), "ledger": ledger, "mesh_quality": mesh, "maxima": maxima, "quality_v4": quality, "generalized_force_v2": gf_v2, "committed_newton": newton, "algorithmic_interface_work_J": work, "first_blocker": first_blocker}
