"""Run Stage 297: continue the accepted 30 s state for 40 s to 70 s."""
...
SOURCE_RUNTIME = ROOT / "runtime" / "295_cpp_worker_precice_three_slice_continue20s_v1"
...
RUNTIME = ROOT / "runtime" / "297_cpp_worker_precice_three_slice_continue40s_v1"
LOGS = RUNTIME / "logs"
RESULTS = ROOT / "results" / "297_cpp_worker_precice_three_slice_continue40s_v1"
RUN_ID = "stage297_cpp_worker_precice_three_slice_continue40s_run_v1"
CASE_ID = "stage297_cpp_worker_precice_three_slice_continue40s_case_v1"
...
<coupling-scheme:parallel-explicit><participants first="Structure_{name}" second="Fluid_{name}"/><time-window-size value="0.005"/><max-time value="40.0"/><exchange data="Displacement" mesh="Structure-Mesh" from="Structure_{name}" to="Fluid_{name}"/><exchange data="Force" mesh="Structure-Mesh" from="Fluid_{name}" to="Structure_{name}"/></coupling-scheme:parallel-explicit></precice-configuration>'''
...
def prepare() -> list[Path]:
    if RUNTIME.exists() or RESULTS.exists():
        raise RuntimeError("refusing to reuse Stage 297 runtime/results")
    ...
    for index in range(3):
        ...
        for child in list(destination.iterdir()):
            if child.name not in {"30", "constant", "system", "precice-config.xml"}:
                if child.is_dir(): shutil.rmtree(child)
                else: child.unlink()
        if not (destination / "30").is_dir():
            raise RuntimeError("30 s source field directory missing")
        control = destination / "system" / "controlDict"
        text = control.read_text(encoding="utf-8")
        text = re.sub(r"startFrom\s+[^;]+;", "startFrom       latestTime;", text)
        text = re.sub(r"startTime\s+[^;]+;", "startTime       30;", text)
        text = re.sub(r"endTime\s+[^;]+;", "endTime         70.0;", text)
...
def main() -> int:
    ...
    shell = " ".join([
        ...
        f"python3 '{participant}' --config '{configs[0]}' '{configs[1]}' '{configs[2]}' --log '{logs}/structure_participant.json' --barrier-log '{logs}/global_barrier.json' --checkpoint-log '{logs}/checkpoint.jsonl' --worker '{worker}' --fixture '{fixture}' --initial-state '{state}' --source-step 6000 --source-time 30.0 --steps 8000 --dt 0.005 --run-id '{RUN_ID}' --case-id '{CASE_ID}' > /dev/null 2> '{logs}/structure.stderr' & spid=\\$!;",
    ...
    schedule = len(steps) == 80 and steps[0:1] == [6100] and steps[-1:] == [14000] and all(b - a == 100 for a, b in zip(steps, steps[1:]))
    ...
    checks = {
        ...
        "committed_target_14000": structure.get("committed_steps") == 14000,
        "local_steps_8000": structure.get("local_committed_steps") == 8000,
        "slice_counts_8000": all(counts.get(f"slice_{i:04d}") == 8000 for i in range(3)),
        "tail_records_20": len(structure.get("tail_records", [])) == 20,
        "checkpoint_count_80": structure.get("checkpoint_count") == 80,
        "checkpoint_schedule_6100_to_14000": schedule,
        ...
        "fluid_final_time_70": all((case / "70").is_dir() for case in cases),
    ...
    gate = {
        "gate_id": "STAGE4F_D_CPP_WORKER_PRECICE_THREE_SLICE_CONTINUE40S_RETRY1_GATE",
        "status": "pass" if run.returncode == 0 and all(checks.values()) else "do_not_pass",
        "stage_id": "stage4f_d_cpp_worker_precice_three_slice_continue40s_retry1_v1",
        "run_id": RUN_ID,
        "case_id": CASE_ID,
        "scope_contract": {"source_step": 6000, "source_time_s": 30.0, "target_step": 14000, "target_time_s": 70.0, "advance_time_s": 40.0, "openfoam": "10", "precice": "3.4.1", "dt_s": 0.005, "slice_count": 3, "storage": "source 30 s fields + purgeWrite=1 + tail 20 + checkpoints every 100 local steps + final restart"},
        "checks": checks,
        "runtime": str(RUNTIME),
        ...
        "qualification": "40 s continuation to 70 s for long-window stability/frequency observation; not formal 15-cycle convergence",
