"""Stage 295 continuation participant: source 10 s -> target 30 s."""
...
def main() -> int:
    ...
    if (args.source_step, args.source_time, args.steps, args.dt) != (2000, 10.0, 4000, 0.005):
        raise SystemExit("Stage 295 requires source step=2000, source time=10, 4000 steps, dt=0.005")
    import precice  # type: ignore

    fixture = json.loads(Path(args.fixture).read_text(encoding="utf-8"))
    initial = json.loads(Path(args.initial_state).read_text(encoding="utf-8"))
    q = tuple(float(x) for x in initial["final_q"])
    qdot = tuple(float(x) for x in initial["final_qdot"])
    qddot = tuple(float(x) for x in initial["final_qddot"])
    model = KernelModel(
