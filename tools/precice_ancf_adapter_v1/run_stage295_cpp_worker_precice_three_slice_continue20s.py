"""Run Stage 295: continue the accepted 10 s state for 20 s to 30 s."""
...
SOURCE_RUNTIME = ROOT / "runtime" / "294_cpp_worker_precice_three_slice_10s_v1"
...
RUNTIME = ROOT / "runtime" / "295_cpp_worker_precice_three_slice_continue20s_v1"
LOGS = RUNTIME / "logs"
RESULTS = ROOT / "results" / "295_cpp_worker_precice_three_slice_continue20s_v1"
RUN_ID = "stage295_cpp_worker_precice_three_slice_continue20s_run_v1"
CASE_ID = "stage295_cpp_worker_precice_three_slice_continue20s_case_v1"
...
<coupling-scheme:parallel-explicit><participants first="Structure_{name}" second="Fluid_{name}"/><time-window-size value="0.005"/><max-time value="20.0"/><exchange data="Displacement" mesh="Structure-Mesh" from="Structure_{name}" to="Fluid_{name}"/><exchange data="Force" mesh="Structure-Mesh" from="Fluid_{name}" to="Structure_{name}"/></coupling-scheme:parallel-explicit></precice-configuration>'''
...
def prepare() -> list[Path]:
    if RUNTIME.exists() or RESULTS.exists():
        raise RuntimeError("refusing to reuse Stage 295 runtime/results")
    ...
    for index in range(3):
        ...
        for child in list(destination.iterdir()):
            if child.name not in {"10", "constant", "system", "precice-config.xml"}:
                if child.is_dir(): shutil.rmtree(child)
                else: child.unlink()
        if not (destination / "10").is_dir():
            raise RuntimeError("10 s source field directory missing")
        control = destination / "system" / "controlDict"
        text = control.read_text(encoding="utf-8")
        text = re.sub(r"startFrom\s+[^;]+;", "startFrom       latestTime;", text)
        text = re.sub(r"startTime\s+[^;]+;", "startTime       10;", text)
        text = re.sub(r"endTime\s+[^;]+;", "endTime         30.0;", text)
...
def main() -> int:
    ...
    participant = f"{project}/tools/precice_ancf_adapter_v1/ancf_cpp_worker_three_slice_continue_v1.py"
    ...
    shell = " ".join([
        ...
        f"python3 '{participant}' --config '{configs[0]}' '{configs[1]}' '{configs[2]}' --log '{logs}/structure_participant.json' --barrier-log '{logs}/global_barrier.json' --checkpoint-log '{logs}/checkpoint.jsonl' --worker '{worker}' --fixture '{fixture}' --initial-state '{state}' --source-step 2000 --source-time 10.0 --steps 4000 --dt 0.005 --run-id '{RUN_ID}' --case-id '{CASE_ID}' > /dev/null 2> '{logs}/structure.stderr' & spid=\\$!;",
    ...
    schedule = len(steps) == 40 and steps[0:1] == [2100] and steps[-1:] == [6000] and all(b - a == 100 for a, b in zip(steps, steps[1:]))
    ...
    checks = {
        ...
        "committed_target_6000": structure.get("committed_steps") == 6000,
        "local_steps_4000": structure.get("local_committed_steps") == 4000,
        "slice_counts_4000": all(counts.get(f"slice_{i:04d}") == 4000 for i in range(3)),
        "tail_records_20": len(structure.get("tail_records", [])) == 20,
        "checkpoint_count_40": structure.get("checkpoint_count") == 40,
        "checkpoint_schedule_2100_to_6000": schedule,
        ...
        "fluid_final_time_30": all((case / "30").is_dir() for case in cases),
    ...
    gate = {
        "gate_id": "STAGE4F_D_CPP_WORKER_PRECICE_THREE_SLICE_CONTINUE20S_V1_GATE",
        "status": "pass" if run.returncode == 0 and all(checks.values()) else "do_not_pass",
        "stage_id": "stage4f_d_cpp_worker_precice_three_slice_continue20s_v1",
        "run_id": RUN_ID,
        "case_id": CASE_ID,
        "scope_contract": {"source_step": 2000, "source_time_s": 10.0, "target_step": 6000, "target_time_s": 30.0, "advance_time_s": 20.0, "openfoam": "10", "precice": "3.4.1", "dt_s": 0.005, "slice_count": 3, "storage": "source 10 s fields + purgeWrite=1 + tail 20 + checkpoints every 100 local steps + final restart"},
        "checks": checks,
        "runtime": str(RUNTIME),
        "source_hashes": {"worker": sha(WORKER), "fixture": sha(SOURCE_FIXTURE), "source_state": sha(SOURCE_STATE), "participant": sha(ROOT / "tools" / "precice_ancf_adapter_v1" / "ancf_cpp_worker_three_slice_continue_v1.py")},
        ...
        "qualification": "20 s continuation to 30 s for stability/frequency observation; not formal 15-cycle convergence",
        "next_authorization": "new explicit authorization required before longer duration or formal statistics",
    }
    RESULTS.mkdir(parents=True, exist_ok=True)
    (RESULTS / "stage4f_d_cpp_worker_precice_three_slice_continue20s_v1_gate.json").write_text(json.dumps(gate, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
