"""Run Stage 296: continue the accepted 30 s state for 40 s to 70 s."""
...
RUNTIME = ROOT / "runtime" / "296_cpp_worker_precice_three_slice_continue40s_v1"
LOGS = RUNTIME / "logs"
RESULTS = ROOT / "results" / "296_cpp_worker_precice_three_slice_continue40s_v1"
RUN_ID = "stage296_cpp_worker_precice_three_slice_continue40s_run_v1"
CASE_ID = "stage296_cpp_worker_precice_three_slice_continue40s_case_v1"
...
def prepare() -> list[Path]:
    if RUNTIME.exists() or RESULTS.exists():
        raise RuntimeError("refusing to reuse Stage 296 runtime/results")
...
def main() -> int:
    ...
    gate = {
        "gate_id": "STAGE4F_D_CPP_WORKER_PRECICE_THREE_SLICE_CONTINUE40S_V1_GATE",
        "status": "pass" if run.returncode == 0 and all(checks.values()) else "do_not_pass",
        "stage_id": "stage4f_d_cpp_worker_precice_three_slice_continue40s_v1",
        "run_id": RUN_ID,
        "case_id": CASE_ID,
