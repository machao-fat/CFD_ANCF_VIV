...
RUN = ROOT / "runtime/fixed_cylinder_current_production_baseline_dt0p0025_60s_v1_run_004"
CASE = RUN / "case"
OUT = ROOT / "results/fixed_cylinder_current_production_baseline_dt0p0025_60s_v1"
...
def build_report(metrics):
    ...
    terminal_text = (f"The latest finite block (cycles {terminal['start_cycle']}--{terminal['end_cycle']}, t={terminal['start_time_s']:.4g}--{terminal['end_time_s']:.4g} s) has means St={terminal['St']['mean']:.7g}, mean CD={terminal['mean_CD']['mean']:.7g}, CL amplitude={terminal['CL_amplitude']['mean']:.7g}; CVs are {terminal['St']['CV']:.3g}, {terminal['mean_CD']['CV']:.3g}, and {terminal['CL_amplitude']['CV']:.3g}. It is reported for finite-window comparison only, not as a confirmed stable zone." if terminal else "No finite terminal cycle block is available.")
...
Statistics discard startup `t<20 s`; complete cycles are detected from positive-going `C_L` zero crossings. {stable_text}
...
No literature-level conclusion is claimed unless a qualifying stable zone exists. The dt=0.005 s baseline had a stable fixed-cylinder lift amplitude above the Shiels fixed-cylinder reference, so this dt=0.0025 s run is intended to quantify temporal sensitivity rather than to explain the free-FSI lift deficit by assumption.

## Final conclusion

`FIXED_CYLINDER_CURRENT_PRODUCTION_BASELINE_DT0P0025_60S = {cls}`

The current 60 s window is sufficient for finite temporal comparison but does not satisfy the predeclared stable-zone CV/trend gate. A same-statistics temporal comparison or longer single dt case would be required before calling this a final temporal-convergence result; no further case is started automatically.
"""
...
def main():
    ...
    metrics = {
        "run_id": "FIXED_CYLINDER_CURRENT_PRODUCTION_BASELINE_DT0P0025_60S_V1_RUN_004",
        ...
        "literature_comparison": comp, "stable_statistics": stable_stats, "terminal_block_statistics": terminal_stats, "dt_sensitivity_comparison": dt_compare, "matched_window_comparison": matched_compare, "shiels_reference": SHIELS, "wake_pattern_visualization": wake,
    ...
    write_csv(OUT / "force_history.csv", DATA, fields); write_csv(OUT / "cycle_statistics.csv", cycles, list(cycles[0]) if cycles else ["cycle"]); dump_json(OUT / "fixed_cylinder_current_production_baseline_dt0p0025_60s_metrics.json", metrics); plot_results(DATA, cycles, stable, quality)
    (OUT / "FIXED_CYLINDER_CURRENT_PRODUCTION_BASELINE_DT0P0025_60S_V1_REPORT.md").write_text(build_report(metrics), encoding="utf-8")
