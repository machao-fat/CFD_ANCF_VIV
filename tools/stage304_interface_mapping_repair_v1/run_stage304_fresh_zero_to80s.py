"""Authorized Stage 304 fresh 0 -> 80 s corrected-mapping run."""
...
RUNTIME = ROOT / "runtime/stage304_interface_mapping_repair_v1_fresh_zero_to80s"
RESULTS = ROOT / "results/304_interface_mapping_repair_v1"
...
PARTICIPANT = ROOT / "tools/stage304_interface_mapping_repair_v1/ancf_cpp_worker_three_slice_mapped_v1.py"
LOGS = RUNTIME / "logs"
RUN_ID = "s304_fresh_zero_to80s_mapping_diag_v1"
CASE_ID = "c304_fresh_zero_to80s_mapping_diag_v1"
DT = 0.005
STEPS = 16000
...
def main() -> int:
    ...
    checks = {
        ...
        "committed_16000": structure.get("target_global_step") == STEPS and structure.get("local_committed_steps") == STEPS,
        "slice_counts_16000": all(structure.get("slice_counts", {}).get(f"slice_{index:04d}") == STEPS for index in range(3)),
        "mapping_diagnostics_16000": len(diagnostics) == STEPS,
    ...
    gate = {
        "gate_id": "STAGE4F_D_INTERFACE_MAPPING_REPAIR_V1_FRESH_ZERO_TO80S_GATE",
        "status": "pass" if run.returncode == 0 and all(checks.values()) else "do_not_pass",
        "stage_id": "stage4f_d_interface_mapping_repair_v1_fresh_zero_to80s",
        "run_id": RUN_ID,
        "case_id": CASE_ID,
        "scope_contract": {"source_step": 0, "source_time_s": 0.0, "target_step": STEPS, "target_time_s": STEPS * DT, "dt_s": DT, "slice_count": 3, "storage": "rolling fields + scalar convergence and mapping diagnostics"},
        "checks": checks,
        ...
        "qualification": "fresh 0-80 s corrected interface mapping with scalar convergence observability; not formal 15-cycle VIV convergence until all statistical and CFD-quality gates pass",
    ...
    (RESULTS / "stage4f_d_interface_mapping_repair_v1_fresh_zero_to80s_gate.json").write_text(json.dumps(gate, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
